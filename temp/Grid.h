...
public:
	...
	bool thisColumnHasLadder(const CellPosition& startPos, const CellPosition& endPos) const; //checks the ladders in the starting cell's column
	Cell* GetCellFromCellPosition(const CellPosition& startPos)const;
	Snake* Grid::GetNextSnake(const CellPosition& position);
	bool thisColumnHasSnake(const CellPosition& startPos, const CellPosition& endPos) const; //checks the snakes in the starting cell's column
	Card* GetCardFromPos(const CellPosition& CardPos);
	void ResetGrid();
