...
bool Grid::AddObjectToCell(GameObject * pNewObject)  // think if any validation is needed
{
	...
	if (pos.IsValidCell()) // Check if valid position
	{
		...
		if( pPrevObject)  // the cell already contains a game object (not null)
...
// Note: You need to change the return type of this function (Think)
...
Snake* Grid::GetNextSnake(const CellPosition& position)
{
	...
	for (int i = position.VCell(); i <= 0; i--) // searching from position.vCell and BELOW
	{
		for (int j = startH; j < NumHorizontalCells; j++) // searching from startH and RIGHT
		{
...
Card* Grid::GetCardFromPos(const CellPosition& position)
{
